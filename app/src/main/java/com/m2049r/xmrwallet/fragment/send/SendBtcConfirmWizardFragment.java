...
public class SendBtcConfirmWizardFragment extends SendWizardFragment implements SendConfirm {
    ...
    public void transactionCreated(final String txTag, final PendingTransaction pendingTransaction) {
        if (isResumed
            ...
            getView().post(() -> {
                hideProgress();
                Timber.d("getFee() SendBtcConfirmWizardFragment ->%s", pendingTransaction.getFee());
                Timber.d("getFee() SendBtcConfirmWizardFragment ->%s", Wallet.getDisplayAmount(pendingTransaction.getFee()));
                Timber.d("getFee() SendBtcConfirmWizardFragment ->%s", pendingTransaction.getAmount());
                Timber.d("getFee() SendBtcConfirmWizardFragment ->%s", Wallet.getDisplayAmount(pendingTransaction.getAmount()));
